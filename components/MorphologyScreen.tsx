...
import { ArrowLeft, ArrowRight, TreePine, Brain, Sparkles, Eye, Star, Search, RotateCcw, Loader2 } from 'lucide-react';
import { Button } from './ui/button';
import { Card, CardContent } from './ui/card';
import { Input } from './ui/input';
import { EtymologyImage } from './EtymologyImage';
import { useGamification } from './GamificationSystem';
import { generateMorphology, isGeminiConfigured } from '../lib/gemini-api';
import { getMorphologyFromCache, saveMorphologyToCache } from '../lib/morpheme-cache';
...
export function MorphologyScreen({ word, onChallengeStart, onBackToSearch, onWordSelect }: MorphologyScreenProps) {
  const [showImage, setShowImage] = useState(true);
  const [useDallE, setUseDallE] = useState(false);
  const [hasAnalyzed, setHasAnalyzed] = useState(false);
  const [quickSearchWord, setQuickSearchWord] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [dynamicMorphology, setDynamicMorphology] = useState(null);
  ...
  const currentWord = dynamicMorphology || morphologyData[word as keyof typeof morphologyData];

  // Gerar análise com Gemini se não existir nos dados estáticos
  useEffect(() => {
    const loadMorphology = async () => {
      console.log('Verificando palavra:', word);
      
      // 1. Verificar dados estáticos primeiro
      if (morphologyData[word as keyof typeof morphologyData]) {
        console.log('Palavra encontrada nos dados estáticos');
        return;
      }
      
      // 2. Verificar cache local
      const cachedAnalysis = getMorphologyFromCache(word);
      if (cachedAnalysis) {
        console.log('Palavra encontrada no cache');
        setDynamicMorphology(cachedAnalysis);
        return;
      }
      
      // 3. Gerar com IA se configurada
      if (isGeminiConfigured()) {
        console.log('Chamando API do Gemini para:', word);
        setIsLoading(true);
        try {
          const analysis = await generateMorphology(word);
          console.log('Análise recebida:', analysis);
          
          // Salvar no cache
          saveMorphologyToCache(word, analysis);
          setDynamicMorphology(analysis);
        } catch (error) {
  ...
  const handleQuickSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (quickSearchWord.trim() && onWordSelect) {
      onWordSelect(quickSearchWord.trim());
      setQuickSearchWord('');
    }
  };

  // Palavras relacionadas baseadas na atual
  const getRelatedWords = (currentWord: string) => {
    const relatedMap = {
      'Filosofia': ['Psicologia', 'Tecnologia'],
      'Democracia': ['Biblioteca', 'Filosofia'],
      'Biblioteca': ['Tecnologia', 'Filosofia'],
      'Psicologia': ['Filosofia', 'Nostalgia'],
      'Tecnologia': ['Filosofia', 'Psicologia'],
      'Nostalgia': ['Psicologia', 'Filosofia']
    };
    return relatedMap[currentWord as keyof typeof relatedMap] || [];
  };
    ...
    return (
      <div className="min-h-screen p-4 sm:p-6 pb-24 flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-8 h-8 animate-spin text-deep-red mx-auto mb-4" />
          <p className="text-sepia font-body">Analisando "<strong>{word}</strong>" com IA...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen p-4 sm:p-6 pb-24">
        <div className="max-w-4xl mx-auto">
          {/* Navegação e busca rápida */}
          <div className="flex flex-col lg:flex-row items-center gap-4 mb-6">
            {/* Botão voltar */}
            <Button
              onClick={onBackToSearch}
              variant="outline"
              className="flex items-center gap-2 text-sepia border-sepia/30 hover:bg-sepia/10 hover:border-sepia/50 transition-colors whitespace-nowrap h-11"
            >
              <ArrowLeft className="w-4 h-4" />
              <span className="font-body">Voltar à Pesquisa</span>
            </Button>
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 pb-24">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-8">
            ...
            <Card className="parchment-card hover-lift morphology-part w-full max-w-xs transform lg:scale-110">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="font-display text-deep-red mb-2">Radical</h3>
                <p className="font-body text-aged mb-2 font-semibold">
        ...
        {getRelatedWords(word).length > 0 && (
          <div className="mb-8">
            <h3 className="font-display text-deep-red text-center mb-4">Palavras Relacionadas</h3>
            <div className="flex flex-wrap justify-center gap-3">
              {getRelatedWords(word).map((relatedWord) => (
                <Button
                  key={relatedWord}
                  onClick={() => onWordSelect?.(relatedWord)}
                  variant="outline"
                  className="text-sepia border-sepia/30 hover:bg-sepia/10 hover:border-sepia/50 transition-colors"
                >
        ...
        {/* Botão de desafio */}
        <div className="text-center mb-8">
          <div className="parchment-card max-w-md mx-auto p-6 bg-gradient-to-br from-aged-paper to-parchment-aged border-2 border-deep-red-light">
            <div className="flex items-center justify-center gap-3 mb-4">
              <Star className="w-6 h-6 text-deep-red" />
              <h3 className="font-display text-deep-red">Próximo Desafio</h3>
              <Star className="w-6 h-6 text-deep-red" />
            </div>
            <p className="text-sepia font-body text-sm mb-6 text-center">
              Teste seus conhecimentos sobre esta palavra em um desafio interativo!
            </p>
            <Button 
              className="btn-primary w-full py-4 text-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105" 
